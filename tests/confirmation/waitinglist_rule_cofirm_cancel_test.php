 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class waitinglist_rule_cofirm_cancel_test extends advanced_testcase {
    /**
     * Tests set up.
     */
     ...
     * @return void
     */
    public function test_confirmation_by_rule_for_option_with_price_but_students_for_free_thnan_cancel(): void {
        global $DB;
        $this->resetAfterTest();
        ...
        $record->minanswers = 1; // We have min users, max users and waitinglist.
        $record->maxanswers = 2;
        ...
        $record->confirmationonnotification = 2; // Confoirm one user at a time.
        ...
        // One user have to be confirmed and 1 must remains on waitinglist - but it not - two still on waitinglist?
        $answers = singleton_service::get_instance_of_booking_answers($settings);
        $bookedusers = $answers->get_usersonlist();
        ...
        foreach ($answers as $answer) {
            $this->assertEquals($answer->status, 5); // Deleted?
        }
    }
